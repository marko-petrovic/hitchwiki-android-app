...
public class Clusterkraf {
	...
	private static GoogleMap mapFinal;
	...
	private static class InnerCallbackListener implements ClusteringOnCameraChangeListener.Host, ClusterTransitionsAnimation.Host, OnMarkerClickListener,
		...
		public void onClusteringCameraChange() {
			...
			float zoom = mapFinal.getCameraPosition().zoom;

			//test
			System.out.println("Options.getMinimalZoomLevel = " + Options.getMinimalZoomLevel());
			
	    	if (zoom < Options.getMinimalZoomLevel())
	    	{
	    		mapFinal.animateCamera(CameraUpdateFactory.zoomTo(Options.getMinimalZoomLevel()), 500, null);
